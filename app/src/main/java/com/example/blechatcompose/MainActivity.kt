...
import com.example.blechatcompose.ble.ChatServer
import com.example.blechatcompose.data.states.DeviceConnectionState
import com.example.blechatcompose.presentation.Chats
import com.example.blechatcompose.viewmodel.DeviceScanViewModel
                        ...
                        Box(
                            ...
                            if (deviceScanningState != null && !isChatOpen || deviceConnectionState == DeviceConnectionState.Disconnected) {
                                Column {
                                    ...
                                    DeviceScans.DeviceScan(deviceScanViewState = deviceScanningState!!) {
                                        isChatOpen = true
                                    }
                                }

                            } else if (deviceScanningState != null && deviceConnectionState is DeviceConnectionState.Connected) {
                                Chats.Chats((deviceConnectionState as DeviceConnectionState.Connected).device.name)
                            } else {
                                Text(text = "Nothing")
                            }
                        }
